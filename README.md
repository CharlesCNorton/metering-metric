...
That changes the theoretical read. The reduced 1+1 sign flip has still **not** been lifted into a raw 3+1D negative-$\kappa$ prediction of the viable branch. But a sign-changing **residual** morphology can now arise within the Einstein-backreaction mainline once the lens geometry is non-spherical and multi-component. The immediate theory task is therefore no longer to ask whether the viable branch can produce any sign-changing structure at all. It is to calibrate that composite Einstein branch against actual cluster geometries and compare its residual morphology directly to the HFF-style archival maps.
...
The Einstein-backreaction branch is in better shape. In the exterior it is a GR-derived branch and relaxes toward Schwarzschild with $\gamma = 1$. That makes it the current mainline candidate. The new theory result is now two-stage. In spherical cluster-style maps, the branch remains convergence-dominated and does not produce a resolved raw sign-flip morphology. But in multi-component composite maps with the same `radial_median_bandpass` style residual used observationally, the branch does produce robust anisotropic sign-changing residual structure while keeping the raw convergence map nonnegative. So the immediate theoretical task has changed again. It is no longer to invent a photon channel, and no longer merely to ask whether multi-component geometry matters. It is to calibrate the viable Einstein branch against realistic cluster geometries and determine whether its residual morphology matches the archival pattern quantitatively rather than just qualitatively.
