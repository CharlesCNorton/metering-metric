...
The workbench now also produces axisymmetric lensing profiles and map summaries for cluster-style source families. Representative outputs are in [`theory_maps/einstein-softened-nfw-map.json`](./theory_maps/einstein-softened-nfw-map.json), [`theory_maps/einstein-beta-map.json`](./theory_maps/einstein-beta-map.json), and [`theory_maps/einstein-gaussian-map.json`](./theory_maps/einstein-gaussian-map.json). The mainline result there is not the old reduced-model sign flip. For the current spherical Einstein-backreaction branch, cluster-like `softened_nfw` and `beta_model` sources remain convergence-dominated with no resolved negative-$\kappa$ annulus. The Gaussian test can show tiny far-tail negative values after exterior matching, but those sit at the numerical floor of the profile and are not a stable structural sign-flip prediction.

That matters. It means the reduced 1+1 convergence-divergence sign flip has **not** yet been lifted into the viable 3+1D mainline branch. If the archival sign-flip-like residual is ultimately real, then within the present theory program it likely requires non-spherical geometry, multi-component structure, line-of-sight projection effects, or a different completion than the current spherical Einstein branch.
...
The Einstein-backreaction branch is in better shape. In the exterior it is a GR-derived branch and relaxes toward Schwarzschild with $\gamma = 1$. That makes it the current mainline candidate. But the new spherical cluster-map outputs also sharpen the limitation of that candidate: in the tested `beta_model` and `softened_nfw` source families, the resulting 3+1D maps remain convergence-dominated and do not reproduce a resolved sign-flip morphology. So the immediate theoretical task has changed. It is no longer to invent a photon channel. It is to carry the Einstein branch into non-spherical or multi-component cluster geometries and test whether the residual morphology can emerge there without leaving the viable branch.
